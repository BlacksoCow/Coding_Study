...
#define MAXLEN 20
using namespace std;
Account* ACCOUNTFUNC::MakeAccount()
{
	int ID, money;
	char name[MAXLEN + 1];
	...
	cin >> money;
	cin.ignore();
	Account* temp = new Account(ID, money, name);
	cout << "Account successfully created";
	cout << endl << endl;
	return temp;
}
void ACCOUNTFUNC::Deposit(Account** account, const int& index)
{
	int ID;
	cout << "[Deposit]" << endl;
	cout << "Input ID: ";
...
void ACCOUNTFUNC::Withdraw(Account** account, const int& index)
{
	int ID;
	cout << "[Withdraw]" << endl;
	cout << "Input ID: ";
...
void ACCOUNTFUNC::ShowAll(Account** account, const int& index)
{
	if(!index)
	{
